...
from Models.MLP import TextProcessingMLP as Mlp
...
tf.flags.DEFINE_string("layers_sizes", "395,134,196,312,363", "Comma-seprated filter sizes (default: '3,4,5'), how many word are convoluted with a filter")
tf.flags.DEFINE_float("dropout_keep_prob", 0.071, "Dropout keep probability (default: 0.5)")
tf.flags.DEFINE_float("learning_rate", 0.002, "Dropout keep probability (default: 1e-3)")

# Training parameters

tf.flags.DEFINE_integer("train_batch_size", 380, "Train batch size (default: 32)")
tf.flags.DEFINE_integer("dev_batch_size", 200, "Train batch size (default: 32)")
tf.flags.DEFINE_integer("num_epochs", 10, "Number of training epochs(default: 2)")
...
if __name__ == '__main__':
	with tf.Graph().as_default():
		...
		with sess.as_default():
			mlp = Mlp.TextProcessingMLP(
								batch_size=FLAGS.train_batch_size,
								max_sequence_length=x_train.shape[1],
								num_classes=len(y_train[0]),
								word_vectors=word_vectors,
								embedding_size=FLAGS.embedding_dim,
								layers_sizes=list(map(int, FLAGS.layers_sizes.split(','))))
			...
			model_name = "_mlp_ls" + str(FLAGS.layers_sizes) + "_lr" + str(FLAGS.learning_rate) + '_drop' + str(FLAGS.dropout_keep_prob) + '_bs' + str(FLAGS.train_batch_size)
			out_dir = os.path.abspath(os.path.join(os.path.curdir, "Models", "MLP", "runs", timestamp + model_name))
			print('Writing to {}\n'.format(out_dir))
			...
			# Checkpoint directory, Tensorflow assumes this directory already exist so we need to create it
			...
			def dev_step(x_batch, y_batch, writer=None):
				...
				step, summaries, loss, accuracy = sess.run([global_step, dev_summary_op, mlp.loss, mlp.accuracy],
																									 feed_dict)
				time_str = datetime.datetime.now().isoformat()
			...
			dev_batches = gb.batch_generator(list(zip(x_dev, y_dev)), FLAGS.train_batch_size, FLAGS.num_epochs * 10)
