...
from Models.RNN import TextProcessingRNN as Rnn
...
tf.flags.DEFINE_integer("max_sequence_length", 250, "max word number in review (default: 250)")
tf.flags.DEFINE_integer("lstm_units", 128, 'lstm units count')
tf.flags.DEFINE_float("dropout_keep_prob", 0.8, "Dropout keep probability (default: 0.5)")

# Training parameters

tf.flags.DEFINE_integer("train_batch_size", 200, "Train batch size (default: 32)")
tf.flags.DEFINE_integer("dev_batch_size", 200, "Train batch size (default: 32)")
tf.flags.DEFINE_integer("num_epochs", 20, "Number of training epochs(default: 2)")
...
if __name__ == '__main__':
	with tf.Graph().as_default():
		...
		with sess.as_default():
			rnn = Rnn.TextProcessingRNN(max_sequence_length = x_train.shape[1],
			...
			optimizer = tf.train.AdamOptimizer(1e-5).minimize(rnn.loss, global_step=global_step)
			...
			model_name = "rnn_lstmu" + str(FLAGS.lstm_units) + "_emdim" + str(FLAGS.embedding_dim) + '_drop' + str(FLAGS.dropout_keep_prob) + '_bs' + str(FLAGS.train_batch_size)
			out_dir = os.path.abspath(os.path.join(os.path.curdir, "Models", "RNN", "runs", timestamp + model_name))
			print('Writing to {}\n'.format(out_dir))
			...
			def train_step(x_batch, y_batch):
				...
				_, step, summaries, loss, accuracy = sess.run(
					[optimizer, global_step, train_summary_op, rnn.loss, rnn.accuracy],
					feed_dict)
			...
			def dev_step(x_batch, y_batch, writer=None):
				...
				step, summaries, loss, accuracy = sess.run([global_step, dev_summary_op, rnn.loss, rnn.accuracy], feed_dict)
				time_str = datetime.datetime.now().isoformat()
